...
export const Environments = new class {
	...
	async handleEvent(event) {
		...
		switch (event.detail.name+' '+event.detail.value) {
			...
			case 'binding removed':
				...
				break;
			case 'open editor':
				const bindingItem = event.detail.data;
				bindingItem.disableRun(true);
				signal = response => {
					this.#openEditor(response);
					bindingItem.disableRun(false);
				}
				IO.sendSignal(false,'cloudflare','start','editor',bindingItem.data,signal);
				break;
			case 'editor closed':
	...
	#openEditor(detail) {
		...
		const value = detail.value == 'checked' ? 'reloaded' : 'started';
		IO.sendSignal(true,context,'editor',value,detail.data);
	}
