...
export const R2Buckets = new class {
	#loaded = false;
	#list = Promise.withResolvers();
	constructor() {
		window.addEventListener('r2_buckets',this,false);
	}
	...
	getList(force = false) {
		if (this.#loaded == false || force) {
			...
			IO.sendSignal(false,'r2','list','buckets',null,signal);
		}
	...
	getBindings() {
		...
		const add = environment => {
			...
			if (scope && scope.r2_buckets) {
				for (const item of scope.r2_buckets) {
					list.push(item.bucket_name);
				}
			}
		}
