...
import { Site,Output,Sitemap,NodeTree,MapData } from '../frontend.js';
...
class GhostNode extends HTMLElement {
	...
	async connectedCallback() {
		this.#template = await Site.getTemplate('ghost-node');
	...
	drop(element,area) {
		...
		if (this.#dragged.data.uni == 'new') {
			const {type,title} = this.#dragged.data;
			this.#dragged = NodeTree.getNewNode(type,title);
		}
		...
		Output.live(null);
		if (this.#dragged && this.#dragged.isConnected) {
			this.#dragged.setState('dragged',false);
		}
		this.#dragged = null;
	}
}
