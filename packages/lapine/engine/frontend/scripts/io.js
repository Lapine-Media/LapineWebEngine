...
import { Site,Output,Overlay,LapineMessage } from './frontend.js';
...
export const IO = new class {
	...
	async handleEvent(event) {
		...
		switch (event.type) {
			case 'load':
				...
				break;
			case 'open':
				this.sendSignal(true,'site','server','ready');
				break;
			case 'message':
				const {context,name,value,data} = JSON.parse(event.data);
				const signal = this.#signalBuffer[context];
				if (context.startsWith('signal_') && signal != undefined) {
					const response = {name,value,data};
					this.receiveSignal(signal,response);
				...
				break;
			case 'close':
				this.sendSignal(true,'site','server','disconnected');
				break;
			case 'error':
	...
	/*getSubmitter(context) {
		const method = this.#signalBuffer[context];
		delete this.#signalBuffer[context];
		return method();
	}*/
